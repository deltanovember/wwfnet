...
*/
	...
	//	Until you edit this function, don't show login, register or logout links
		...
		return array(
			'login' => 'http://wordswithfriends.net/?page_id=386',
			'register' => 'http://wordswithfriends.net/?page_id=386',
			'logout' => 'http://wordswithfriends.net/?page_id=386'
		);
...
*/
	...
	//	Until you edit this function, nobody is ever logged in
		...
		if (is_logged_in ()) {
                    ...
                    $screen_name = get_username_from_cookie();
                    $user = get_user($screen_name);
                    return array(
					'userid' => $user->get_id(),
					'publicusername' => $screen_name,
					'email' => $user->get_email(),
					'level' => $user->get_permissions() == 5 ? QA_USER_LEVEL_ADMIN : QA_USER_LEVEL_BASIC
				);
	...
	//	Until you edit this function, always return null
		...
		if (count($publicusernames)) {
                    ...
                    foreach ($publicusernames as $publicusername) {
                        $user = get_user($publicusername);
                        $publictouserid[$publicusername]=$user->get_id();
                    }
	...
	//	Until you edit this function, always return null
		...
		if (count($userids)) {
                    ...
                    foreach ($userids as $userid) {
                        $user = get_user_from_id($userid);
                        $publictousername[$userid]=$user->get_screen_name();
                    }

		}

		return $publictousername;
