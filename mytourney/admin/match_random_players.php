...
$query = $query = "SELECT * FROM `$t_id` WHERE onround = $onround AND r" . $onround . "_vs IS NULL AND status = 'Active' AND player <> '$player' ORDER BY RAND() LIMIT 1";
//print $query;
//die();
$result = mysql_query($query);
$count=mysql_num_rows($result);
if($count>0){} else {
header("Location: admin_index.php?msg=no_matches");
}
...
<form action="match_random_players.php?<? echo "id=" . $id . "&t_id=" . $t_id . "&round=" . $onround; ?>" method="post" name="form" id="form">
...
echo "<br /><form method=GET action='match_random_players.php'><input type=hidden name='t_id' value='" . $t_id . "'><input type=hidden name='id' value='" . $id . "'><input type='Submit' value='Click to Continue'></form></center>";
}
