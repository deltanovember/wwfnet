...
function increment_round() {
    $query = "SELECT * FROM T_CONTROL WHERE `t_status` = 'In Progress' AND `t_status` <> 'Complete' AND `t_id` LIKE('%BAM%') ORDER BY `t_start` DESC";
...
function is_blocked_matchup($user_id1, $user_id2, $tournament_id, $round) {
    ...
    $num_matches_1 = countDailyMatches($user_id1, $tournament_id);
    $num_matches_2 = countDailyMatches($user_id2, $tournament_id);
...
// filter by player activity in this round and last roud
...
// filter ranked results by valid players
while ($row= mysql_fetch_array($result)) {
    $user_id = $row['user_id'];
