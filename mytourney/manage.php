...
$query = "INSERT INTO `$t_id` (  `id` ,  `player` ,  `skill` ) VALUES ('$id',  '$username',  '$skill');";
$results = mysql_query( $query ) or die("Couldn't save info to the database" . mysql_error() ); 
}
...
// If player has been eliminated, send him back to index
...
//if ($opponent == "" && $t_status != "Register"){
if ($opponent != ""){
}else{
header("Location: pickplayer.php?t_id=" . $t_id . "&id=" . $id);}
...
if ($p1_rslt == 'Won') {$rslt_message = "<p style='color:green;'><b>YOU ARE THE WINNER FOR ROUND " . $onround . "! Please wait for the next round to begin.</b></p>";} 
elseif ($p1_rslt == 'Lost' || $p1_rslt == 'Forfeited' || $p1_rslt == 'No-Show' || $p1_rslt == 'Incomplete') {$rslt_message = "<p><span style='font-weight:bold; color:red;'>You lost in Round " . $onround . "</span>. We will send you an email when registration opens for the next tournament.</b></p>";}
else {}
...
<h3>Your Round <?php echo $onround; ?> Opponent is '<?php echo $firstname1 ?>'</h3>
<?php
if ($url1 != "") {
...
echo "
<p>
<strong>WWF Screen Name</strong>: <span style='color:green; font-weight:bold;'>" . $username1 . "</span><br />" . $has_url1 .
...
<h3 style='background-color:#CCCCCC; border-top:1px solid #000000; border-bottom:1px solid #000000; padding:10px;'>STEP 1: Make Contact with '<?php echo $firstname1 ?>'</h3>
...
<table width="500px" border="0" cellspacing="0" cellpadding="0" style="padding: 10px; border: 1px solid #CCC;">
    <form action="manage.php?t_id=<?php echo $t_id; ?>&id=<?php echo $id ?>" method="post" name="form" id="form">
...
This short message (140 chars max) is sent via email to your opponent. It also
    ...
    </td><td><textarea cols="25" rows="4" name="private_msg"></textarea><div align="right"><input type="hidden" name="send_pmessage" value="true"><input name="submit" <?php if ($mailbody != "") {echo "disabled";} ?> type="submit" value="Send" /></div></td></tr>
...
<h3 style='background-color:#CCCCCC; border-top:1px solid #000000; border-bottom:1px solid #000000; padding:10px;'>STEP 2: Record Round <?php echo $onround; ?> Results</h3>

<form action="manage.php?t_id=<?php echo $t_id; ?>&id=<?php echo $id ?>" method="post" name="form" id="form">

<table cellpadding="3" width="500px" cellspacing="2" style="border-collapse:collapse; border:1px solid #CCC;">

<tr><td>&nbsp;</td><td style="font-weight:bold;"><?php echo $player; ?></td><td style="font-weight:bold;"><?php echo $opponent; ?></td></tr>
<tr>
<td><b>Game 1</b></td>
...
<tr>
<td><b>Game 2</b></td>
<td><input type = "text" value="<?php echo $p1_g2; ?>" name="p1_g2" width="5" /></td>
<td><input type = "text" value="<?php echo $p2_g2; ?>" name="p2_g2" width="5" /></td>
</tr>

<tr>
<td><b>Game 3</b></td>
<td><input type = "text" value="<?php echo $p1_g3; ?>" name="p1_g3" width="5" /></td>
<td><input type = "text" value="<?php echo $p2_g3; ?>" name="p2_g3" width="5" /></td>
</tr>
<?php if ($p2_cmt != "") {
...
?>
<tr valign="top"><td colspan="2"><p><b>Comment on this Round (140 max):<br /><br />
<span style="color:red;">NOTE: This will be immediately visible to everyone publicly on the Results tab.</span></b></p></td><td><textarea cols="25" rows="4" name="new_cmt"><?php echo $p1_cmt ?></textarea><div align="right"><input type="submit" value="Update" name="update" /></div></td></tr>
</table>
<h3 style='background-color:#CCCCCC; border-top:1px solid #000000; border-bottom:1px solid #000000; padding:10px;'>STEP 3: Report Final Outcome</h3>
...
else {echo "<div style='color:red; padding:5px; font-weight:bold;'>Complete this section only after there is a final resolution to the entire round.</div>";} 
?>

<table cellpadding="3" width="500px" cellspacing="2" style="border-collapse:collapse; border:1px solid #CCC;">

<tr valign="top"><td colspan="2" align="right"><p><b>Who won Round <?php echo $onround; ?>?</b></p></td><td width="65%"><p>
...
<input type="radio" id="<?php echo $opponent; ?>" <?php if ($p2_rslt == 'Won') {echo 'checked="checked"';} ?> name="winner" value="<?php echo $opponent; ?>" /><?php echo $opponent; ?>
</p>
</td></tr> 
<tr valign="top"><td colspan="2" align="right"><p><b>How Did Round <?php echo $onround; ?> End?<br /></b></p></td><td><p><select name="result" >
...
<tr valign="top"><td colspan="2" align="right"><p><b>Rate Your Experience* with this Player:<br /></b></p></td><td><p><select name="rating" >
  <option value="">--Make Selection--</option>
...
<div style="text-align:center; margin-top:20px;"><input type="submit" value="Report Winner" name="update" /></div>
</form>
</div> 
</div>
</body>
