...
?>
<script type="text/javascript" src="http://wordswithfriends.net/wp-content/themes/vibe/scripts.js"></script>
...
$query = "SELECT * FROM $db." . $currentTourney . "";
$result = mysql_query($query) or die("Couldn't execute query");
$count=mysql_num_rows($result);
echo "<script type='text/javascript'>var playerCount = '" . $count . "';</script>";
$query = "SELECT player FROM $db." . $currentTourney . " ORDER BY `record` DESC LIMIT 1";

$result = mysql_query($query) or die("Couldn't execute query");
$row= mysql_fetch_array($result);
$player= $row["player"];
echo "<script type='text/javascript'>var lastJoined = '" . $player . "';</script>";
$query = "SELECT * FROM $db." . $currentTourney . " WHERE status = 'Active'";
$result = mysql_query($query) or die("Couldn't execute query");
$count=mysql_num_rows($result);
...
<script type="text/javascript">
    document.write(t_desc_m + " - Round " + onround_m + '<BR/>');

//document.write(activeCount + " out of " + playerCount + " players in this round");
document.write(playerCount + " players in this round" + "<BR/><BR/>");
